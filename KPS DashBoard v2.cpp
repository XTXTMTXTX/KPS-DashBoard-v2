...
#define WINDOWNAME "KPS DashBoard v2"
	...
	while (msg.message!=WM_QUIT){
		...
		if (::PeekMessage(&msg,0,0,0,PM_REMOVE)){
			...
			if(_device){
				...
				D3DXMatrixTranslation(&TransMatrix,1.25-1.25f,0.5-0.5f,0.0f);  
				...
				if(KB==100){
					...
					D3DXMatrixTranslation(&TransMatrix,1.25-1.3812,0.5-0.935,0.0f); 
					ResultMatrix=ScalMatrix*TransMatrix;
					_device->SetTransform(D3DTS_WORLD, &ResultMatrix); 
					_device->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,0,0,4,0,2);
					D3DXMatrixScaling(&ScalMatrix,0.215,0.215,1.5f);
					D3DXMatrixTranslation(&TransMatrix,1.25-1.10322,0.5-0.935,0.0f); 
					ResultMatrix=ScalMatrix*TransMatrix;
					_device->SetTransform(D3DTS_WORLD, &ResultMatrix); 
					_device->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,0,0,4,0,2);
				}else{
